...
Codex Kimetsu Skin 通过本机回环 CDP 给官方 Codex Windows 桌面应用加载外部主题。这个 fork 只内置我妻善逸「霹雳呼吸」主题；原生侧栏、项目选择、任务内容和输入框仍然可交互，不修改 WindowsApps、`app.asar` 或应用签名。
...
安装器会校验官方 Codex Store 包和 Node.js，保存可恢复的外观配置，并初始化本地主题仓库。首次安装会把「霹雳呼吸」设为活动主题，并保存为 `preset-thunder-breathing`。默认还会创建这些快捷方式：

- `Codex Kimetsu Skin`：启动或重新应用皮肤。
- `Codex Kimetsu Skin - Tray`：打开系统托盘主题控制。
- `Codex Kimetsu Skin - Restore`：恢复官方外观并关闭已保存的 CDP 会话。
...
推荐从 `Codex Kimetsu Skin` 快捷方式启动。它发现 Codex 已经运行时会先询问是否重启。
...
打开 `Codex Kimetsu Skin - Tray` 后可以：

- 更换 PNG、JPEG 或 WebP 背景图。
- 保存当前主题并从「已保存主题」切换；内置项只有「霹雳呼吸」。
- 暂停或继续显示皮肤。
- 重新应用主题，或完整恢复 Codex。
...
如需同时删除 Kimetsu Skin 创建的快捷方式，再增加 `-Uninstall`：
...
| 用途 | 路径 |
|------|------|
| Kimetsu Skin 状态根目录 | `%LOCALAPPDATA%\CodexKimetsuSkin` |
| 当前主题 | `%LOCALAPPDATA%\CodexKimetsuSkin\active-theme` |
| 已保存主题 | `%LOCALAPPDATA%\CodexKimetsuSkin\themes` |
| 导入图片归档 | `%LOCALAPPDATA%\CodexKimetsuSkin\images` |
| 会话状态 | `%LOCALAPPDATA%\CodexKimetsuSkin\state.json` |
| 注入器日志 | `%LOCALAPPDATA%\CodexKimetsuSkin\injector.log` |
| 注入器错误日志 | `%LOCALAPPDATA%\CodexKimetsuSkin\injector-error.log` |
| 验证日志 | `%LOCALAPPDATA%\CodexKimetsuSkin\verify.log` |
...
通过 `Codex Kimetsu Skin` 快捷方式启动 Codex，再运行验证脚本。普通 Codex 启动方式不会打开 Kimetsu Skin 所需的调试会话。

### Codex 更新后皮肤失效

重新运行安装器和启动快捷方式。脚本会重新发现当前注册的 Store 包，不依赖旧版本的可执行文件路径。

提交问题时请附上系统版本、Codex 来源、复现步骤和相关日志片段。请删除密钥、`auth.json`、中转 token 和私人对话内容。
