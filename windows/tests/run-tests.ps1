...
$temporaryRoot = Join-Path ([System.IO.Path]::GetTempPath()) "codex-kimetsu-skin-tests-$PID-$([guid]::NewGuid().ToString('N'))"
...
try {
  ...
  $engine = Install-KimetsuSkinRuntimeEngine -SkillRoot $runtimeSourceRoot -StateRoot $runtimeStateRoot
  ...
  if ($runtimeSourceFiles.Count -ne $runtimeEngineFiles.Count -or
    -not (Test-KimetsuSkinPathWithin -Path $engine.Start -Root $runtimeStateRoot) -or
    -not (Test-KimetsuSkinPathWithin -Path $engine.Restore -Root $runtimeStateRoot) -or
    -not (Test-KimetsuSkinPathWithin -Path $engine.Tray -Root $runtimeStateRoot)) {
  ...
  $realRuntimeCleanup = (Get-Command Remove-KimetsuSkinRuntimeTree -CommandType Function).ScriptBlock
  ...
  try {
    ...
    if (-not $runtimeCleanupFailure.Triggered -or $runtimeUpdateReportedFailure -or
      ...
      (Read-KimetsuSkinUtf8File -Path (Join-Path $engine.Root 'scripts\runtime-update.test')) -cne 'updated') {
      throw 'Runtime reinstall did not commit cleanly when old-engine cleanup failed.'
    }
  } finally {
    $WarningPreference = $previousWarningPreference
    Set-Item -Path Function:\Remove-KimetsuSkinRuntimeTree -Value $realRuntimeCleanup
  }
  foreach ($runtimeBackup in Get-ChildItem -LiteralPath $runtimeStateRoot -Directory -Force |
    ...
    Remove-KimetsuSkinRuntimeTree -Path $runtimeBackup.FullName -StateRoot $runtimeStateRoot
  }
  ...
  Remove-Item -LiteralPath (Join-Path $invalidRuntimeRoot 'scripts\start-kimetsu-skin.ps1') -Force
  $invalidRuntimeRejected = $false
  try {
    $null = Install-KimetsuSkinRuntimeEngine -SkillRoot $invalidRuntimeRoot -StateRoot $runtimeStateRoot
  } catch {
  ...
  try {
    $null = Install-KimetsuSkinRuntimeEngine -SkillRoot $runtimeSourceRoot -StateRoot $nestedStateRoot
  } catch {
  ...
  $installSource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'scripts\install-kimetsu-skin.ps1')
  $trayGuardIndex = $installSource.IndexOf('if (Test-KimetsuSkinTrayActive)', [System.StringComparison]::Ordinal)
  $engineInstallIndex = $installSource.IndexOf('$engine = Install-KimetsuSkinRuntimeEngine', [System.StringComparison]::Ordinal)
  ...
  Write-KimetsuSkinUtf8FileAtomically -Path $atomicReplacePath -Content 'after'
  if ((Read-KimetsuSkinUtf8File -Path $atomicReplacePath) -cne 'after') {
  ...
  $realAtomicCleanup = (Get-Command Remove-KimetsuSkinAtomicArtifact -CommandType Function).ScriptBlock
  $previousWarningPreference = $WarningPreference
  ...
  try {
    ...
    try {
      Write-KimetsuSkinUtf8FileAtomically -Path $cleanupFailurePath -Content 'after'
    } catch {
      $cleanupFailureReported = $true
    }
    if (-not $cleanupFailure.Triggered -or $cleanupFailureReported -or
      (Read-KimetsuSkinUtf8File -Path $cleanupFailurePath) -cne 'after') {
    ...
    Install-KimetsuSkinBaseTheme -ConfigPath $cleanupConfigPath -BackupPath $cleanupBackupPath
    if (-not (Test-Path -LiteralPath $cleanupBackupPath) -or
      -not (Test-KimetsuSkinBytesEqual -Left $cleanupOriginalBytes `
    ...
    $WarningPreference = $previousWarningPreference
    Set-Item -Path Function:\Remove-KimetsuSkinAtomicArtifact -Value $realAtomicCleanup
  }

  $configPath = Join-Path $temporaryRoot 'config.toml'
  $backupPath = Join-Path $temporaryRoot 'config.before-kimetsu-skin.toml'
  ...
  $installed = Read-KimetsuSkinUtf8File -Path $configPath
  ...
  if (-not (Test-Path -LiteralPath (Get-KimetsuSkinAppearanceMarkerPath -BackupPath $backupPath))) {
  ...
  Write-KimetsuSkinUtf8FileAtomically -Path $configPath -Content $installed
  Restore-KimetsuSkinBaseTheme -ConfigPath $configPath -BackupPath $backupPath
  $restored = Read-KimetsuSkinUtf8File -Path $configPath
  ...
  $legacyCurrent = "[desktop]`r`n$($script:KimetsuSkinLegacyAppearanceTheme)`r`n$($script:KimetsuSkinManagedLightCodeTheme)`r`n$($script:KimetsuSkinManagedLightChromeTheme)`r`n"
  ...
  Install-KimetsuSkinBaseTheme -ConfigPath $legacyConfigPath -BackupPath $legacyBackupPath
  $legacyMigrated = Read-KimetsuSkinUtf8File -Path $legacyConfigPath
  ...
  Write-KimetsuSkinUtf8FileAtomically -Path $legacyConfigPath -Content $legacyMigrated
  Restore-KimetsuSkinBaseTheme -ConfigPath $legacyConfigPath -BackupPath $legacyBackupPath
  if ((Read-KimetsuSkinUtf8File -Path $legacyConfigPath) -notmatch 'appearanceTheme = "dark"') {
  ...
  Install-KimetsuSkinBaseTheme -ConfigPath $lfConfigPath -BackupPath $lfBackupPath
  $lfInstalled = Read-KimetsuSkinUtf8File -Path $lfConfigPath
  ...
  Restore-KimetsuSkinBaseTheme -ConfigPath $lfConfigPath -BackupPath $lfBackupPath
  $lfRestored = Read-KimetsuSkinUtf8File -Path $lfConfigPath
  ...
  Install-KimetsuSkinBaseTheme -ConfigPath $quotedConfigPath -BackupPath $quotedBackupPath
  $quotedInstalled = Read-KimetsuSkinUtf8File -Path $quotedConfigPath
  ...
  Restore-KimetsuSkinBaseTheme -ConfigPath $quotedConfigPath -BackupPath $quotedBackupPath
  if ((Read-KimetsuSkinUtf8File -Path $quotedConfigPath) -cne $quotedOriginal) {
  ...
  Install-KimetsuSkinBaseTheme -ConfigPath $nestedConfigPath -BackupPath $nestedBackupPath
  $nestedInstalled = Read-KimetsuSkinUtf8File -Path $nestedConfigPath
  $nestedDesktop = Get-KimetsuSkinDesktopSection -Content $nestedInstalled
  ...
  Restore-KimetsuSkinBaseTheme -ConfigPath $nestedConfigPath -BackupPath $nestedBackupPath
  if ((Read-KimetsuSkinUtf8File -Path $nestedConfigPath) -cne $nestedOriginal) {
  ...
  Install-KimetsuSkinBaseTheme -ConfigPath $singleLineArrayPath -BackupPath $singleLineArrayBackup
  if (-not (Read-KimetsuSkinUtf8File -Path $singleLineArrayPath).Contains($singleLineArray.TrimEnd())) {
  ...
  foreach ($unsupported in @(
    ...
    try { Install-KimetsuSkinBaseTheme -ConfigPath $unsupportedPath -BackupPath $unsupportedBackup } catch { $unsupportedRejected = $true }
  ...
  Write-KimetsuSkinUtf8FileAtomically -Path $configPath -Content 'intentionally changed'
  Restore-KimetsuSkinConfigBackup -ConfigPath $configPath -BackupPath $backupPath -RecoveryBackupPath $recoveryPath
  ...
  if ((Read-KimetsuSkinUtf8File -Path $recoveryPath) -cne 'intentionally changed') {
  ...
  Archive-KimetsuSkinConfigBackup -BackupPath $backupPath -ArchivePath $archivePath
  ...
  Install-KimetsuSkinBaseTheme -ConfigPath $configPath -BackupPath $backupPath
  if (-not (Test-KimetsuSkinBytesEqual -Left $secondBaselineBytes -Right ([System.IO.File]::ReadAllBytes($backupPath)))) {
  ...
  $rejected = $false
  try { Install-KimetsuSkinBaseTheme -ConfigPath $invalidPath -BackupPath $invalidBackupPath } catch { $rejected = $true }
  ...
  $utf16Rejected = $false
  try { Install-KimetsuSkinBaseTheme -ConfigPath $utf16Path -BackupPath $utf16BackupPath } catch { $utf16Rejected = $true }
  ...
  try { Install-KimetsuSkinBaseTheme -ConfigPath $utf16NoBomPath -BackupPath $utf16NoBomBackupPath } catch { $utf16NoBomRejected = $true }
  ...
  $raceRejected = $false
  try { Assert-KimetsuSkinFileUnchanged -Path $racePath -ExpectedBytes $raceExpected } catch { $raceRejected = $true }
  if (-not $raceRejected) { throw 'Concurrent config modification was not detected.' }
  $conditionalWriteRejected = $false
  try {
    Write-KimetsuSkinUtf8FileAtomically -Path $racePath -Content 'replacement' -ExpectedBytes $raceExpected
  } catch {
    $conditionalWriteRejected = $true
  }
  if (-not $conditionalWriteRejected -or (Read-KimetsuSkinUtf8File -Path $racePath) -cne 'after') {
    throw 'Conditional atomic write replaced newer config content.'
  }

  if (-not (Test-KimetsuSkinWebSocketUrl -Value 'ws://127.0.0.1:9335/devtools/page/test' -Port 9335)) {
  ...
  foreach ($unsafe in @(
    ...
    if (Test-KimetsuSkinWebSocketUrl -Value $unsafe -Port 9335) { throw "Accepted unsafe CDP target: $unsafe" }
  }
  ...
  if (-not (Test-KimetsuSkinCdpPageTarget -Target $safePageTarget -Port 9335)) {
  ...
  foreach ($unsafePageTarget in @(
    ...
    if (Test-KimetsuSkinCdpPageTarget -Target $unsafePageTarget -Port 9335) {
      throw 'Accepted an inconsistent CDP page target.'
    }
  }
  $watchCommand = '"C:\Program Files\nodejs\node.exe" "C:\Kimetsu Skin\injector.mjs" --watch --port 9335 --browser-id browser-123'
  if (-not (Test-KimetsuSkinCommandLineToken -CommandLine $watchCommand -Token 'C:\Kimetsu Skin\injector.mjs') -or
    (Test-KimetsuSkinCommandLineToken -CommandLine $watchCommand -Token 'Kimetsu Skin\injector.mjs')) {
    throw 'Injector command-line token validation is not boundary-safe.'
  }
  if (-not (Test-KimetsuSkinBrowserId -Value 'browser-123') -or
    (Test-KimetsuSkinBrowserId -Value 'browser 123')) {
    throw 'CDP browser ID validation is not boundary-safe.'
  }
  $quotedProfile = ConvertTo-KimetsuSkinProcessArgument -Value '--user-data-dir=C:\Kimetsu Skin\Profile\'
  if ($quotedProfile -cne '"--user-data-dir=C:\Kimetsu Skin\Profile\\"') {
    throw 'Process argument quoting did not protect spaces and a trailing backslash.'
  }
  $argumentLine = ConvertTo-KimetsuSkinArgumentLine -Arguments @(
    '--remote-debugging-address=127.0.0.1',
    '--user-data-dir=C:\Kimetsu Skin\Profile\',
    ''
  )
  if ($argumentLine -cne '--remote-debugging-address=127.0.0.1 "--user-data-dir=C:\Kimetsu Skin\Profile\\" ""') {
    throw 'Packaged-app argument line quoting failed.'
  }
  Initialize-KimetsuSkinPackageLauncher
  if (-not ('CodexKimetsuSkin.PackageLauncher' -as [type])) {
    throw 'Packaged-app activation helper did not compile.'
  }
  $invalidActivationRejected = $false
  try { $null = Start-KimetsuSkinCodex -Codex ([pscustomobject]@{ AppUserModelId = 'invalid app' }) } catch {
  ...
  $state = [pscustomobject]@{
    ...
    injectorPath = 'C:\Kimetsu Skin\injector.mjs'
  ...
  $loadedState = Read-KimetsuSkinState -Path $statePath
  ...
  Write-KimetsuSkinState -Path $statePath -State $missingIdentityState
  $missingIdentityRejected = $false
  try { $null = Read-KimetsuSkinState -Path $statePath } catch { $missingIdentityRejected = $true }
  ...
  Write-KimetsuSkinState -Path $statePath -State $legacyState
  if ((Read-KimetsuSkinState -Path $statePath).schemaVersion -ne 2) {
  ...
  $fakeInstall = ConvertTo-KimetsuSkinCodexInstall -Package $fakePackage -Manifest $fakeManifest
  if ($null -eq $fakeInstall -or $fakeInstall.PackageFullName -cne $fakePackage.PackageFullName -or
    ...
    -not (Test-KimetsuSkinPathEqual -Left $fakeInstall.Executable -Right $fakeExecutable)) {
    throw 'Registered Appx package identity conversion failed.'
  }
  $fakeManifest.Package.Applications.Application[1].Id = 'Invalid App'
  if ($null -ne (ConvertTo-KimetsuSkinCodexInstall -Package $fakePackage -Manifest $fakeManifest)) {
  ...
  if ($null -eq (Get-KimetsuSkinCodexStatePathCandidate -State $pathOnlyState)) {
    throw 'A structurally valid legacy Codex path was not recognized for read-only activity checks.'
  }
  if ($null -eq (Resolve-KimetsuSkinCodexInstallFromState -State $pathOnlyState `
  ...
  $resolvedInstall = Resolve-KimetsuSkinCodexInstallFromState -State $verifiedPackageState `
    -RegisteredInstalls @($fakeInstall)
  ...
  if ($null -ne (Resolve-KimetsuSkinCodexInstallFromState -State $verifiedPackageState `
    -RegisteredInstalls @($fakeInstall))) {
    throw 'A mismatched Appx package family was accepted from state.'
  }
  Write-KimetsuSkinUtf8FileAtomically -Path $statePath -Content '[]'
  $badStateRejected = $false
  try { $null = Read-KimetsuSkinState -Path $statePath } catch { $badStateRejected = $true }
  ...
  $themePaths = Initialize-KimetsuSkinThemeStore -SkillRoot $Root -StateRoot $themeStateRoot
  $initialTheme = Read-KimetsuSkinTheme -ThemeDirectory $themePaths.Active
  if ($initialTheme.Theme.id -cne 'preset-thunder-breathing' -or
    $initialTheme.Theme.name -cne '霹雳呼吸' -or
    ...
    throw 'Default Windows theme did not seed the Thunder Breathing wallpaper contract.'
  }
  $preseededThemes = @(Get-KimetsuSkinSavedThemes -StateRoot $themeStateRoot)
  if ($preseededThemes.Count -ne 1 -or
    $preseededThemes[0].Id -cne 'preset-thunder-breathing' -or
    $preseededThemes[0].Name -cne '霹雳呼吸') {
    throw 'Thunder Breathing was not preseeded in the Windows saved-theme menu.'
  }
  $updatedTheme = Set-KimetsuSkinActiveTheme -ImagePath (Join-Path $Root 'assets\kimetsu-reference.jpg') `
  ...
  if ($updatedTheme.Theme.name -cne '测试主题' -or
    ...
    -not (Test-KimetsuSkinThemePathWithin -Path $updatedTheme.ImagePath -Root $themePaths.Active)) {
    throw 'Imported image did not reset to the generic adaptive contract inside the managed directory.'
  }
  $obsoletePreset = Join-Path $themePaths.Saved 'preset-obsolete'
  New-Item -ItemType Directory -Path $obsoletePreset | Out-Null
  Copy-Item -LiteralPath (Join-Path $Root 'assets\kimetsu-reference.jpg') `
    -Destination (Join-Path $obsoletePreset 'kimetsu-reference.jpg')
  Copy-Item -LiteralPath (Join-Path $Root 'assets\theme.json') `
    -Destination (Join-Path $obsoletePreset 'theme.json')
  $null = Initialize-KimetsuSkinThemeStore -SkillRoot $Root -StateRoot $themeStateRoot
  $idempotentTheme = Read-KimetsuSkinTheme -ThemeDirectory $themePaths.Active
  if ($idempotentTheme.Theme.id -cne 'custom' -or
    @(Get-KimetsuSkinSavedThemes -StateRoot $themeStateRoot).Count -ne 1 -or
    (Test-Path -LiteralPath $obsoletePreset)) {
    throw 'Theme-store initialization overwrote the active custom theme, duplicated its bundled preset, or preserved an obsolete preset.'
  }
  $savedTheme = Save-KimetsuSkinCurrentTheme -Name '已保存主题' -StateRoot $themeStateRoot
  if ($savedTheme.Theme.name -cne '已保存主题' -or @(Get-KimetsuSkinSavedThemes -StateRoot $themeStateRoot).Count -ne 2) {
    throw 'Saved theme creation or discovery failed.'
  }
  $null = Use-KimetsuSkinSavedTheme -ThemeDirectory $savedTheme.Directory -StateRoot $themeStateRoot

  $outsideTheme = Join-Path $temporaryRoot 'outside-theme'
  New-Item -ItemType Directory -Path $outsideTheme | Out-Null
  Copy-Item -LiteralPath (Join-Path $Root 'assets\kimetsu-reference.jpg') `
    -Destination (Join-Path $outsideTheme 'kimetsu-reference.jpg')
  Copy-Item -LiteralPath (Join-Path $Root 'assets\theme.json') `
  ...
  try {
    $null = Use-KimetsuSkinSavedTheme -ThemeDirectory $junctionTheme -StateRoot $themeStateRoot
  ...
  Set-KimetsuSkinPaused -Paused $true -StateRoot $themeStateRoot | Out-Null
  if (-not (Test-KimetsuSkinPaused -StateRoot $themeStateRoot)) { throw 'Pause marker was not created.' }
  Set-KimetsuSkinPaused -Paused $false -StateRoot $themeStateRoot | Out-Null
  if (Test-KimetsuSkinPaused -StateRoot $themeStateRoot) { throw 'Pause marker was not removed.' }
  ...
  Write-KimetsuSkinUtf8FileAtomically -Path (Join-Path $oversizedTheme 'theme.json') `
    -Content "{`"image`":`"oversized.jpg`"}`r`n"
  $oversizedReadRejected = $false
  try { $null = Read-KimetsuSkinTheme -ThemeDirectory $oversizedTheme } catch { $oversizedReadRejected = $true }
  $oversizedSetRejected = $false
  try {
    $null = Set-KimetsuSkinActiveTheme -ImagePath $oversizedImage -Theme $null -StateRoot $themeStateRoot
  ...
  try { $null = Set-KimetsuSkinActiveTheme -ImagePath $oversizedDimensionImage -Theme $null -StateRoot $themeStateRoot } catch { $oversizedDimensionRejected = $true }
  ...
  try { $null = Initialize-KimetsuSkinThemeStore -SkillRoot $Root -StateRoot $reparseStateRoot } catch { $reparseInitRejected = $true }
  ...
  $css = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'assets\kimetsu-skin.css')
  foreach ($requiredCss in @(
    'background-image: var(--kimetsu-art)',
    ...
    '--kimetsu-immersive-composer',
    'background-position: var(--kimetsu-art-position)',
    '.kimetsu-home-utility',
    ':has(.kimetsu-home-utility) .composer-surface-chrome',
    ':is(.kimetsu-task-ambient, .kimetsu-task-banner):has(main.main-surface:not(.kimetsu-home-shell))'
  )) {
    if (-not $css.Contains($requiredCss)) { throw "Windows immersive CSS is missing: $requiredCss" }
  }
  $traySource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'scripts\tray-kimetsu-skin.ps1')
  ...
  if (-not $traySource.Contains('Read-KimetsuSkinTheme -ThemeDirectory $paths.Active -SkipImageMetadata') -or
    -not $traySource.Contains('Get-KimetsuSkinSavedThemes -StateRoot $StateRoot -SkipImageMetadata')) {
    throw 'Tray menu metadata enumeration still performs full image parsing on every open.'
  }
  $restoreSource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'scripts\restore-kimetsu-skin.ps1')
  if (-not $restoreSource.Contains('Stop-KimetsuSkinTrayProcess')) {
  ...
  if ($restoreSource.Contains('Start-Process -FilePath $relaunchCodex.Executable') -or
    -not $restoreSource.Contains('Start-KimetsuSkinCodex -Codex $relaunchCodex')) {
    throw 'Restore still executes the WindowsApps path instead of activating the registered package.'
  }
  $startSource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'scripts\start-kimetsu-skin.ps1')
  if ($startSource.Contains('Start-Process -FilePath $codex.Executable') -or
    -not $startSource.Contains('Start-KimetsuSkinCodex -Codex $codex')) {
    throw 'Start still executes the WindowsApps path instead of activating the registered package.'
  }
  $stateReadIndex = $startSource.IndexOf('$previousState = Read-KimetsuSkinState', [System.StringComparison]::Ordinal)
  $restartPromptIndex = $startSource.IndexOf('$restartAuthorized = Confirm-KimetsuSkinRestart', [System.StringComparison]::Ordinal)
  $recordedStopIndex = $startSource.IndexOf('$recordedInjectorStopped = Stop-KimetsuSkinRecordedInjector', [System.StringComparison]::Ordinal)
  $cancelIndex = $startSource.IndexOf("Write-Host 'Kimetsu Skin launch was cancelled", [System.StringComparison]::Ordinal)
  $pauseClearIndex = $startSource.IndexOf('Set-KimetsuSkinPaused -Paused $false', [System.StringComparison]::Ordinal)
  ...
  if (-not $startSource.Contains('$pauseWasSet = Test-KimetsuSkinPaused') -or
    -not $startSource.Contains('$pauseCleared = $true') -or
    -not $startSource.Contains('Set-KimetsuSkinPaused -Paused $true -StateRoot $StateRoot')) {
    throw 'Start does not preserve an existing pause marker when startup rolls back.'
  }

  $rendererSource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'assets\renderer-inject.js')
  foreach ($requiredRendererBehavior in @('kimetsu-home-utility', 'artMetadata', 'detectShellAppearance')) {
  ...
  $injectorSource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'scripts\injector.mjs')
  ...
  $themeSource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'scripts\theme-windows.ps1')
  foreach ($requiredThemeSafety in @(
    '[System.IO.FileAttributes]::ReparsePoint',
    'Ensure-KimetsuSkinManagedDirectory',
    'Get-KimetsuSkinValidatedImageMetadata',
  ...
  $commonSource = Read-KimetsuSkinUtf8File -Path (Join-Path $Root 'scripts\common-windows.ps1')
  ...
  $stderrProbe = Invoke-KimetsuSkinNative -FilePath $node.Path -ArgumentList @(
    '-e', "process.stderr.write('kimetsu-skin-stderr-probe\n'); process.exit(7)")
  if ($stderrProbe.ExitCode -ne 7 -or ($stderrProbe.Output -join "`n") -notmatch 'kimetsu-skin-stderr-probe') {
  ...
  $discardedProbe = Invoke-KimetsuSkinNative -FilePath $node.Path -ArgumentList @(
  ...
  $oversizedPayloadTest = Invoke-KimetsuSkinNative -FilePath $node.Path -ArgumentList @(
  ...
  $rendererTest = Invoke-KimetsuSkinNative -FilePath $node.Path -ArgumentList @(
  ...
  $bootstrapTest = Invoke-KimetsuSkinNative -FilePath $node.Path -ArgumentList @(
  ...
  $oneShotTest = Invoke-KimetsuSkinNative -FilePath $node.Path -ArgumentList @(
  ...
  $imageMetadataTest = Invoke-KimetsuSkinNative -FilePath $node.Path -ArgumentList @(
