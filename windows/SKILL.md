...
description: Apply, launch, verify, repair, update, or restore the Thunder Breathing decorative skin for the Windows Codex desktop app. Use when the user asks for this fork's Codex theme, needs the skin reapplied after a Codex update, or needs a safe rollback without modifying WindowsApps or app.asar.
---
...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-kimetsu-skin.ps1` once. The installer preserves the user's native appearance settings, seeds the Thunder Breathing theme, copies the runtime to `%LOCALAPPDATA%\CodexKimetsuSkin\engine`, and creates launch/restore/tray shortcuts that do not depend on the source checkout.
2. Use the `Codex Kimetsu Skin` shortcut, or run `%LOCALAPPDATA%\CodexKimetsuSkin\engine\scripts\start-kimetsu-skin.ps1`. The shortcut asks before restarting an already-open Codex app; CLI callers must explicitly add `-RestartExisting`.
3. Run `scripts/verify-kimetsu-skin.ps1 -ScreenshotPath <absolute-path>` after launch. Treat a missing continuous wallpaper, home shell, native composer, sidebar layer, or injection marker as failure. The native suggestion count is responsive and may be two to four.
...
- Keep the injection daemon running for navigation/reload resilience. Its state and logs live under `%LOCALAPPDATA%\CodexKimetsuSkin`.
...
- The active theme, saved themes, imported images, pause marker, and tray controls live under `%LOCALAPPDATA%\CodexKimetsuSkin`. Reject empty or over-16 MB images before copying or encoding them.
...
- Treat `%LOCALAPPDATA%\CodexKimetsuSkin\engine` as an installer-managed runtime. Exit the Kimetsu Skin tray before reinstalling so the installer can replace that runtime atomically and update every shortcut to the same copy.
...
- `assets/kimetsu-skin.css`: full visual layer.
- `assets/renderer-inject.js`: idempotent DOM integration and cleanup.
- `assets/kimetsu-reference.jpg`: pure Thunder Breathing wallpaper seeded as the default and as a saved theme; it contains no Codex UI.
...
- `scripts/tray-kimetsu-skin.ps1`: Windows Forms tray for apply, pause, import, save, switch, and complete restore.
