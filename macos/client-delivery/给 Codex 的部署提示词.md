...
2. 解压后的客户目录根部应能看到：
   ...
   - `使用说明.txt`
   - `给 Codex 的部署提示词.md`
   完整引擎位于隐藏目录 `.codex-kimetsu-skin`。这是正常结构，不要删除、改名或只复制其中的 CSS/图片。Finder 默认看不到隐藏目录时，直接从终端使用其绝对路径。
...
4. 确认官方 Codex 至少运行过一次，且 `~/.codex/config.toml` 已存在。运行：
   `<ENGINE>/scripts/install-kimetsu-skin-macos.sh --no-launch`
   完整项目应被安装到 `~/.codex/codex-kimetsu-skin`，并生成桌面启动、定制、验证和恢复入口。

5. 如果我上传了主题图片，使用安装后的脚本处理素材：
   `~/.codex/codex-kimetsu-skin/scripts/customize-theme-macos.sh --image "<图片绝对路径>" --name "我的 Codex Kimetsu Skin" --no-apply`
...
7. 启动后必须运行：
   - `~/.codex/codex-kimetsu-skin/scripts/doctor-macos.sh --require-live`
   - `~/.codex/codex-kimetsu-skin/scripts/verify-kimetsu-skin-macos.sh --reload --screenshot "<首页验收截图绝对路径>"`
   验证器必须真实返回 `pass: true`。随后还要检查一个正常任务页面，确认背景存在且正文、菜单、侧栏和输入框仍清晰可用，并保存任务页截图。

8. 检查桌面已存在以下四个入口：
   - `Codex Kimetsu Skin.command`
   - `Codex Kimetsu Skin - Customize.command`
   - `Codex Kimetsu Skin - Verify.command`
   - `Codex Kimetsu Skin - Restore.command`

9. 如果失败，读取 `~/Library/Application Support/CodexKimetsuSkin/` 下的日志并继续修复。不得降低代码签名、回环端口归属、PID 身份、原生结构或截图验证标准；不得用“预计重启后生效”“应该完成”等措辞代替验收。
