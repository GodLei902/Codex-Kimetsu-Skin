...
if [ "$IN_PLACE" = "false" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  exec "$INSTALL_ROOT/scripts/install-kimetsu-skin-macos.sh" "${install_args[@]}"
fi
...
if [ ! -f "$THEME_DIR/theme.json" ]; then
  "$SCRIPT_DIR/switch-theme-macos.sh" --id preset-thunder-breathing --no-apply >/dev/null
fi
...
write_launcher() {
  local target="$1"
  local command="$2"
  if [ -e "$target" ] && ! /usr/bin/grep -q '^# CodexKimetsuSkin launcher$' "$target" 2>/dev/null; then
    fail "Refusing to overwrite an unrelated Desktop file: $target"
  fi
  /usr/bin/printf '%s\n' \
    '#!/bin/bash' \
    '# CodexKimetsuSkin launcher' \
    'set -e' \
    "$command" > "$target"
...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  start_script="$(shell_quote "$SCRIPT_DIR/start-kimetsu-skin-macos.sh")"
  customize_script="$(shell_quote "$SCRIPT_DIR/customize-theme-macos.sh")"
  verify_script="$(shell_quote "$SCRIPT_DIR/verify-kimetsu-skin-macos.sh")"
  restore_script="$(shell_quote "$SCRIPT_DIR/restore-kimetsu-skin-macos.sh")"
  screenshot="$(shell_quote "$HOME/Desktop/Codex Kimetsu Skin Verification.png")"
  write_launcher "$HOME/Desktop/Codex Kimetsu Skin.command" "exec $start_script --port $PORT --prompt-restart"
  write_launcher "$HOME/Desktop/Codex Kimetsu Skin - Customize.command" "exec $customize_script"
  write_launcher "$HOME/Desktop/Codex Kimetsu Skin - Verify.command" "$verify_script --screenshot $screenshot && /usr/bin/open $screenshot"
  write_launcher "$HOME/Desktop/Codex Kimetsu Skin - Restore.command" "exec $restore_script --restore-base-theme --restart-codex"
fi

printf 'Codex Kimetsu Skin %s installed at %s for Codex %s using its signed Node.js %s.\n' \
...
if [ "$LAUNCH_AFTER_INSTALL" = "true" ]; then
  "$SCRIPT_DIR/start-kimetsu-skin-macos.sh" --port "$PORT" --prompt-restart
fi
