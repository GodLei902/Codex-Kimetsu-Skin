...
// Kimetsu Skin CSS auto-adapts to light/dark via data-kimetsu-shell.
...
async function acquireConfigLock() {
  const lockPath = `${configPath}.kimetsu-skin.lock`;
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error("Another Kimetsu Skin config operation is still running; try again shortly.");
      }
