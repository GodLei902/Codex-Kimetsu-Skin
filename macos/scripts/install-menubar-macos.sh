...
PLUGIN_SRC="$PROJECT_ROOT/menubar/codex_kimetsu_skin.10s.sh"
...
PLUGIN_DST="$PLUGIN_DIR/codex_kimetsu_skin.10s.sh"
{
  printf '%s\n' '#!/bin/bash'
  printf 'export CODEX_KIMETSU_SKIN_ENGINE=%q\n' "$ENGINE_ROOT"
...
if [ -d "$INSTALL_ROOT" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  /bin/cp -f "$PLUGIN_SRC" "$INSTALL_ROOT/menubar/codex_kimetsu_skin.10s.sh"
  /bin/chmod 755 "$INSTALL_ROOT/menubar/codex_kimetsu_skin.10s.sh"
  for name in pause-kimetsu-skin-macos.sh status-kimetsu-skin-macos.sh apply-from-menubar-macos.sh \
...
/bin/chmod 755 \
  "$PROJECT_ROOT/scripts/pause-kimetsu-skin-macos.sh" \
  "$PROJECT_ROOT/scripts/status-kimetsu-skin-macos.sh" \
