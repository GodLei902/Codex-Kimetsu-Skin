...
if [ "$RESET_DEMO" = "true" ]; then
  ...
  if [ -z "$THEME_NAME" ]; then
    THEME_NAME="$(/usr/bin/osascript -e 'text returned of (display dialog "给这套主题起个名字" default answer "我的 Codex Kimetsu Skin" buttons {"取消", "继续"} default button "继续")')" \
...
if [ "$APPLY_NOW" = "true" ]; then
  "$SCRIPT_DIR/start-kimetsu-skin-macos.sh" --port 9341 --prompt-restart
fi

printf 'Codex Kimetsu Skin theme is ready.\n'
