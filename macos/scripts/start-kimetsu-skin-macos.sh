...
record_start_error() {
  ...
  printf 'Codex Kimetsu Skin: start failed at line %s (exit %s). See %s\n' "$line" "$code" "$START_ERROR_LOG" >&2
}
...
if codex_is_running && [ "$DEBUG_READY" = "false" ]; then
  if [ "$PROMPT_RESTART" = "true" ] && [ "$RESTART_EXISTING" = "false" ]; then
    /usr/bin/osascript -e 'display dialog "Codex 需要重启一次才能启用 Kimetsu Skin。" buttons {"取消", "重启并应用"} default button "重启并应用" with title "Codex Kimetsu Skin"' >/dev/null \
...
VERIFY_OUTPUT="$(/usr/bin/mktemp "${TMPDIR:-/tmp}/kimetsu-skin-verify.XXXXXX")"
...
if [ "$verify_code" -ne 0 ]; then
  ...
  if /usr/bin/grep -q '"installed": true' "$VERIFY_OUTPUT" 2>/dev/null; then
    printf 'Codex Kimetsu Skin %s is active (soft verify) on port %s.\n' "$SKIN_VERSION" "$PORT"
...
printf 'Codex Kimetsu Skin %s is active on loopback port %s.\n' "$SKIN_VERSION" "$PORT"
