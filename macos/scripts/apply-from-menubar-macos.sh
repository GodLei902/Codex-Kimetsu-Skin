...
on run argv
  ...
  display dialog promptText buttons {"取消", okLabel} default button okLabel with title "Codex Kimetsu Skin"
end run
APPLESCRIPT
}
...
"$SCRIPT_DIR/start-kimetsu-skin-macos.sh" --port "$PORT" --restart-existing >>"$LOG_OUT" 2>&1
code=$?
