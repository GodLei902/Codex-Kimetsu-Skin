...
if [ -z "${HOME:-}" ]; then
  ...
  [ -n "$HOME" ] || { printf 'Codex Kimetsu Skin: could not resolve the current macOS home directory.\n' >&2; exit 1; }
  export HOME
fi
...
INSTALL_ROOT="$HOME/.codex/codex-kimetsu-skin"
STATE_ROOT="$HOME/Library/Application Support/CodexKimetsuSkin"
...
CODEX_APP_JOB_LABEL="com.openai.codex-kimetsu-skin.app"
INJECTOR_JOB_LABEL="com.openai.codex-kimetsu-skin.injector"
...
on run argv
  display notification (item 1 of argv) with title "Codex Kimetsu Skin"
end run
APPLESCRIPT
}
...
alert_user() {
  ...
  /usr/bin/osascript - "$message" <<'APPLESCRIPT' >/dev/null 2>&1 || true
on run argv
  display alert "Codex Kimetsu Skin" message (item 1 of argv)
end run
APPLESCRIPT
}
...
seed_bundled_presets() {
  ...
  for dest in "$themes_root"/preset-*/; do
    [ -d "$dest" ] || continue
    id="$(/usr/bin/basename "$dest")"
    [ -f "$presets_root/$id/theme.json" ] || /bin/rm -rf "$dest"
  done
...
stop_recorded_injector() {
  ...
  if ! pid="$(state_field injectorPid 2>/dev/null)" || [ -z "${pid:-}" ]; then
    printf 'Kimetsu Skin state is damaged or missing its injector PID; state was preserved.\n' >&2
    return 1
  fi
  ...
  case "$pid" in
    *[!0-9]*|??????????*)
      printf 'Recorded Kimetsu Skin injector PID is invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  ...
  case "$saved_port" in
    ''|*[!0-9]*)
      printf 'Recorded Kimetsu Skin injector port is missing or invalid; state was preserved.\n' >&2
      return 1
      ;;
  esac
  [ "$saved_port" -ge 1024 ] && [ "$saved_port" -le 65535 ] || {
    printf 'Recorded Kimetsu Skin injector port is out of range; state was preserved.\n' >&2
    return 1
  }
  if [ -z "$saved_start" ] || [ -z "$saved_node" ] || [ -z "$saved_injector" ]; then
    printf 'Recorded Kimetsu Skin injector identity is incomplete; state was preserved.\n' >&2
    return 1
  fi
  ...
  if recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    printf 'Could not stop the recorded Kimetsu Skin injector (PID %s).\n' "$pid" >&2
    return 1
  fi
  return 0
}
