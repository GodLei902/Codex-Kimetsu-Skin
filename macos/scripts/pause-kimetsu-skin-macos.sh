...
if [ "$REMOVED" = "true" ]; then
  printf 'Codex Kimetsu Skin paused (skin removed; Codex left running). Port %s may still be in debug mode.\n' "$PORT"
elif codex_is_running; then
  printf 'Codex Kimetsu Skin paused (injector stopped). Live remove skipped: CDP on port %s not verified.\n' "$PORT"
else
  printf 'Codex Kimetsu Skin paused (Codex is not running).\n'
fi
